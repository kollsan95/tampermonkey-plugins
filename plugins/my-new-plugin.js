...
(function() {
    ...
    // Флаг, показывающий, было ли уже открыто окно после обнаружения сломанных зон
    ...
    function getTokenFromCookies() {
        // Пробуем получить токен из document.cookie
    ...
    function getTokenFromStorage() {
        // Пробуем получить токен из localStorage или sessionStorage
        try {
    ...
    function getToken() {
        ...
        // Если в куках нет — пробуем из storage
        ...
        // Если ничего не нашли — пробуем через GM_cookie (если доступно)
        try {
            if (typeof GM_cookie !== 'undefined') {
                // Это асинхронно, поэтому возвращаем null, а токен будем получать отдельно
                console.log('ℹ️ My Plugin: Используется GM_cookie для получения токена');
            }
        } catch (e) {
            console.warn('⚠️ My Plugin: GM_cookie недоступен');
        }
    ...
    // Если страница не подходит — не регистрируем плагин
    if (!isValidPage()) {
    ...
    function loadData(container) {
        ...
        // Делаем два запроса: для карты зала и для информации о сессии
        ...
        console.log(`📡 My Plugin: Запрос к API карты: ${mapUrl}`);
        console.log(`📡 My Plugin: Запрос к API сессии: ${sessionUrl}`);
        ...
        // Запрос 1: Карта зала (с авторизацией)
        ...
        // Запрос 2: Информация о сессии (с авторизацией)
    ...
    function processData(container, mapData, sessionData) {
        ...
        // Сортируем группы по количеству
        ...
        // ============================================================
        // 10. ОБРАБОТЧИКИ КЛИКОВ (раскрытие плиток)
        // ============================================================
        ...
        // ============================================================
        // 11. СБРАСЫВАЕМ УВЕДОМЛЕНИЕ ПОСЛЕ ОТКРЫТИЯ ОКНА
        // ============================================================
    ...
    // ============================================================
    // 12. РЕГИСТРАЦИЯ ПЛАГИНА
    // ============================================================

    wasOpenedAfterBrokenFound = false;
